...
def test_scatter_out(x_size: tuple, batch_size: int, dim: int, reduce_op: str):
    ...
    s_custom = scatter(x, batch, dim=dim, reduce=reduce_op)
    s_custom_comp = compiled_scatter(x, batch, dim=dim, reduce=reduce_op)
    ...
    torch.testing.assert_close(
        ...
        atol=1e-5,
        rtol=1e-5,
        msg="PYG and Customs Compiled results differ",
    )
...
def test_gradient_scatter_out(x_size: tuple, batch_size: int, dim: int, reduce_op: str):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    x = torch.rand(x_size, device=device, dtype=torch.float32)
    batch = torch.arange(
        batch_size, dtype=torch.int64, device=device
    ...
    x = x.detach().requires_grad_(True)
    gs_custom_comp = torch.autograd.grad(
...
def test_scatter_softmax_out(x_size: tuple, batch_size: int, dim: int):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    x = torch.rand(x_size, device=device, dtype=torch.float32)
    batch = torch.arange(
        batch_size, dtype=torch.int64, device=device
    ...
    s_custom = scatter_softmax(x, batch, dim=dim)
    s_custom_comp = compiled_scatter_softmax(x, batch, dim=dim)
...
def test_gradient_scatter_softmax_out(x_size: tuple, batch_size: int, dim: int):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    x = torch.rand(x_size, device=device, dtype=torch.float32)
    batch = torch.arange(
        batch_size, dtype=torch.int64, device=device
